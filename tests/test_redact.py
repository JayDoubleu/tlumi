...
def test_redact_no_redos_on_repeated_begin_markers():
    """_GCP_PRIVATE_KEY must scale linearly. Repeated BEGIN markers with no END would
    make an unbounded lazy '.*?' expand to end-of-string at each marker (O(n^2)); the
    bounded '.{0,8192}?' keeps it linear. Doubling the input should at most roughly
    double the runtime, so a quadratic regression (~4x) trips the assert. Ratio-based
    rather than an absolute wall-clock bound, so it is independent of CI runner speed
    (the bounded scan has a large constant factor that put the old 1.0s bound right on
    the edge on loaded runners)."""
    import time

    def best_elapsed(markers: int) -> float:
        big = "-----BEGIN PRIVATE KEY----- " * markers  # repeated BEGIN, never an END
        best = float("inf")
        for _ in range(3):
            start = time.monotonic()
            redact_text(big)
    ...
    t_small = best_elapsed(2000)
    t_large = best_elapsed(4000)
    # Linear doubling is ~2x; a catastrophic-backtracking regression is ~4x or worse.
    # 3x sits between the two with slack for timer noise.
    assert t_large < t_small * 3, (
        f"redact_text _GCP_PRIVATE_KEY should be linear: {t_small:.3f}s -> {t_large:.3f}s"
    )
