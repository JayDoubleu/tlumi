...
1. **tlumi forwards your shell environment to the engine.** When tlumi runs
   Pulumi it overrides the `PULUMI_*` variables it manages (plus
   `PYTHONDONTWRITEBYTECODE`) and blanks its own `TLUMI_SECRETS_PASSPHRASE` and
   `TLUMI_VAR_*` variables. Every other
